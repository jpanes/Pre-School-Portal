class AveragesController < ApplicationController
  ...
  def index1
    @averages = Average.where("grading == ?", 1)
  end

  def index2
    @averages = Average.where("grading == ?", 2)
  end

  def index3
    @averages = Average.where("grading == ?", 3)
  end

  def index4
    @averages = Average.where("grading == ?", 4)
  end
  ...
  def destroy
    @average.destroy
    respond_to do |format|
      format.html { redirect_to averages_url, notice: 'Student was successfully destroyed.' }
  ...
  private
    ...
    def average_params
      params.require(:average).permit(:name, :filipino, :english, :science, :math, :msep, :grading)
    end
end
